import {
    Asset, Request, Handover, Dismantle, Customer, User, Division, AssetCategory, StandardItem
} from '../types';
import {
  initialMockRequests,
  mockAssets,
...
function getFromStorage<T>(key: string, defaultValue: T): T {
    try {
        ...
        // FIX: Handle cases where localStorage might contain the string "undefined" or is null.
        if (storedValue && storedValue !== 'undefined') {
            return JSON.parse(storedValue);
        }
    } catch (error) {
        // Log the error but allow the app to continue with default data.
        console.error(`Gagal mem-parsing localStorage untuk kunci "${key}":`, error);
    }
    
    // If we reach here, it's because storage is empty, invalid, or "undefined".
...
export const fetchAllData = () => {
    return apiCall(() => {
        ...
        return { assets, requests, handovers, dismantles, customers, users, divisions, assetCategories };
    });
};
