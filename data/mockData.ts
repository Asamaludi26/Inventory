...
const ispAssetCategories: Record<string, { types: Partial<AssetType>[], isCustomerInstallable?: boolean, associatedDivisions?: number[] }> = {
    'Perangkat Jaringan (Core)': {
        types: [
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Router Core', standardItems: [{ id: 0, name: 'Router Core RB4011iGS+', brand: 'Mikrotik' }, { id: 0, name: 'EdgeRouter Pro', brand: 'Ubiquiti' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Switch Distribusi', standardItems: [{ id: 0, name: 'CRS326-24G-2S+RM', brand: 'Mikrotik' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'OLT', standardItems: [{ id: 0, name: 'OLT EPON 8 Port', brand: 'Huawei' }, { id: 0, name: 'OLT GPON 16 Port', brand: 'ZTE' }] },
        ],
    ...
    'Perangkat Pelanggan (CPE)': {
        types: [
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'ONT/ONU', standardItems: [{ id: 0, name: 'ONT HG8245H', brand: 'Huawei' }, { id: 0, name: 'ONT F609', brand: 'ZTE' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Router WiFi', standardItems: [{ id: 0, name: 'Router WiFi Archer C6', brand: 'TP-Link' }, { id: 0, name: 'Router WiFi AX10', brand: 'TP-Link' }] },
        ],
    ...
    'Infrastruktur Fiber Optik': {
        types: [
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Kabel Dropcore', trackingMethod: 'bulk', unitOfMeasure: 'roll', baseUnitOfMeasure: 'meter', quantityPerUnit: 150, standardItems: [{ id: 0, name: 'Kabel Dropcore 1 Core 150m', brand: 'FiberHome' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Kabel UTP', trackingMethod: 'bulk', unitOfMeasure: 'box', baseUnitOfMeasure: 'meter', quantityPerUnit: 305, standardItems: [{ id: 0, name: 'Kabel UTP Cat6 305m', brand: 'Belden' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Konektor', trackingMethod: 'bulk', unitOfMeasure: 'pack', baseUnitOfMeasure: 'pcs', quantityPerUnit: 100, standardItems: [{ id: 0, name: 'Konektor Fast Connector SC', brand: 'Generic' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'ODP', standardItems: [{ id: 0, name: 'ODP 16 Core', brand: 'Generic' }] },
        ],
        associatedDivisions: [3] // Engineer
    },
    'Alat Kerja Lapangan': {
        types: [
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Fusion Splicer', standardItems: [{ id: 0, name: 'Fusion Splicer 90S', brand: 'Fujikura' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'OTDR', standardItems: [{ id: 0, name: 'OTDR AQ7280', brand: 'Yokogawa' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Power Meter', standardItems: [{ id: 0, name: 'Optical Power Meter', brand: 'Joinwit' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Laptop Teknisi', standardItems: [{ id: 0, name: 'Laptop ThinkPad T480', brand: 'Lenovo' }] },
        ],
        associatedDivisions: [3] // Engineer
    },
    'Aset Kantor': {
        types: [
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'PC Desktop', standardItems: [{ id: 0, name: 'PC Rakitan Core i7', brand: 'Custom' }, { id: 0, name: 'PC Dell Optiplex', brand: 'Dell' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Monitor', standardItems: [{ id: 0, name: 'Monitor LG 24 inch', brand: 'LG' }] },
            // FIX: Added dummy id to satisfy StandardItem type
            { name: 'Printer', standardItems: [{ id: 0, name: 'Printer Epson L3210', brand: 'Epson' }] },
        ],
    ...
    for (let i = 0; i < ASSET_COUNT; i++) {
        ...
        if (status === AssetStatus.IN_USE) {
            ...
            if (currentUser && !currentUser.startsWith('TMI-')) {
                ...
                allHandovers.push({
                    id: handoverId, handoverDate: handoverDate.toISOString().split('T')[0], menyerahkan: 'Alice Johnson', penerima: currentUser, mengetahui: 'John Doe', woRoIntNumber: `INT-${id}`, lembar: '1. Menyerahkan',
...
export const initialMockRequests: Request[] = Array.from({ length: REQUEST_COUNT }, (_, i) => {
    ...
    const requestDate = new Date(new Date(NOW).setDate(NOW.getDate() - (REQUEST_COUNT - i)));
    ...
    const allModels = initialAssetCategories.flatMap(c => c.types.flatMap(t => t.standardItems || []));
    const requestedModel = allModels[i % allModels.length];
    ...
    const request: Request = {
        ...
        items: [{
            id: 1,
            itemName: requestedModel.name,
            itemTypeBrand: requestedModel.brand,
