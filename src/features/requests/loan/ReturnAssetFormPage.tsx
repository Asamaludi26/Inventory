...
import { User, Asset, LoanRequest, AssetCondition, AssetReturn, Division, AssetReturnStatus } from '../../../types';
...
interface ReturnAssetFormPageProps {
    ...
    assetToReturn?: Asset;
    onSave: (data: Omit<AssetReturn, 'id' | 'docNumber' | 'status'>) => void;
    onCancel: () => void;
    users: User[];
    divisions: Division[];
    returns: AssetReturn[];
    ...
    onRejectReturn?: (returnId: string, reason: string) => void;
}
...
const ReturnAssetFormPage: React.FC<ReturnAssetFormPageProps> = (props) => {
    const { currentUser, loanRequest, assetToReturn, onSave, onCancel, users, divisions, returns, isReadOnly = false, returnDocument, onApproveReturn, onRejectReturn } = props;

    // Use state for form mode, derive values for read-only mode
    ...
    const data = isReadOnly ? returnDocument : {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (isReadOnly || !loanRequest || !assetToReturn) {
        ...
        <div className="p-4 sm:p-6 md:p-8">
                ...
                <form id={formId} onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                        ...
                        <div>
                            ...
                            <p className="font-medium text-gray-800">{loanRequest?.requester || '-'}</p>
                        </div>
                        ...
                        <div>
                            <span className="font-semibold text-gray-500">Tanggal Peminjaman:</span>
                                <p className="font-medium text-gray-800">
                                    {loanRequest ? new Date(loanRequest.requestDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'}
                                </p>
                        </div>
                    ...
                    <fieldset disabled={isReadOnly}>
                        ...
                        <section className="mt-6 pt-6 border-t pb-1 mb-4">
                             <h4 className="font-semibold text-gray-800 border-b pb-1 mb-4">Aset Dikembalikann</h4>
                                ...
                                <table className="min-w-full text-left text-sm">
                                            ...
                                            <tbody>
                                                <tr className="border-b">
                                                    ...
                                                    <td className="p-3 font-mono text-gray-600 break-words">{assetToReturn?.macAddress || '-'}</td>
                                                    
                                                    <td className="p-3 font-semibold text-gray-800">{data.returnedCondition}</td>
                                                </tr>
                                            </tbody>
                                </table>
                    ...
                    {canApprove && (
                        <div className="p-4 bg-blue-50 border-t border-b border-blue-200 space-y-3 no-print">
                            ...
                            <div className="flex gap-4">
                                <button type="button" onClick={() => onApproveReturn!(returnDocument!.id)} disabled={isSubmitting} className="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-success rounded-lg shadow-sm hover:bg-green-700">Setujui</button>
            ...
            <Modal isOpen={isRejectModalOpen} onClose={() => setIsRejectModalOpen(false)} title="Tolak Pengembalian Aset">
                ...
                <div className="flex justify-end gap-2 mt-4">
                    ...
                    <button onClick={() => { onRejectReturn!(returnDocument!.id, rejectionReason); setIsRejectModalOpen(false); }} disabled={!rejectionReason.trim()} className="px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg shadow-sm hover:bg-red-700 disabled:bg-red-300">Kirim Penolakan</button>
                </div>
            </Modal>
        </div>
    );
