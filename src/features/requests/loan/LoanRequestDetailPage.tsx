...
const AssignmentPanel: React.FC<{
    ...
    const handleStartScan = (itemId: number, index: number) => {
        ...
        setFormScanCallback(() => (data: ParsedScanResult) => {
            ...
            if (assetIdToSet) {
                 const allAssigned = Object.values(itemsState).flatMap(s => s.assignedAssets);
    ...
    const handleSubmit = () => {
        ...
        for (const item of request.items) {
            ...
            if (state.approvedQty > 0) {
                if (state.assignedAssets.some(a => !a)) {
                    addNotification(`Harap pilih aset untuk semua slot yang disetujui pada ${item.itemName}.`, 'error');
                    isValid = false;
                    break;
                }
                const uniqueAssets = new Set(state.assignedAssets);
                if (uniqueAssets.size !== state.assignedAssets.length) {
            ...
            let status = 'approved';
            if (state.approvedQty === 0) status = 'rejected';
            else if (state.approvedQty < item.quantity) status = 'partial';

            resultItemStatuses[item.id] = {
                status,
                reason: state.reason,
                approvedQuantity: state.approvedQty
            };
            
            if (state.approvedQty > 0) {
                resultAssignedAssets[item.id] = state.assignedAssets;
            }
        }
        ...
        <div className="bg-white border-2 border-blue-100 rounded-xl shadow-xl animate-fade-in-up relative">
            ...
            <div className="p-6 space-y-8">
                {request.items.map(item => {
                    ...
                    const allCurrentlyAssigned = Object.values(itemsState).flatMap(s => s.assignedAssets).filter(Boolean);
