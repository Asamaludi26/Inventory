...
const StatusAndActionSidebar: React.FC<RequestDetailPageProps & {
    ...
    const {
        ...
        onLogisticApproval, onOpenRejectionModal, onOpenCancellationModal, onOpenFollowUpModal,
    ...
    const isLogisticsAdmin = currentUser.role === 'Admin Logistik';
    const isPurchaseAdmin = currentUser.role === 'Admin Purchase';
    ...
    const renderActions = () => {
        ...
        switch (request.status) {
            case ItemStatus.PENDING:
                ...
                if (isLogisticsAdmin || isSuperAdmin) {
                    return (
                        <div className="space-y-2">
                            ...
                            <ActionButton onClick={onOpenRejectionModal} disabled={isLoading} text="Revisi / Tolak" color="secondary" icon={PencilIcon} />
                        </div>
                    );
                }
                break;
            ...
            case ItemStatus.LOGISTIC_APPROVED:
                if (isPurchaseAdmin) {
                    const actions = [
                        ...
                        <ActionButton key="reject" onClick={onOpenRejectionModal} disabled={isLoading} text="Revisi / Tolak" color="secondary" icon={PencilIcon} />
                    ];
                ...
                if (isSuperAdmin && !request.isPrioritizedByCEO) {
            ...
            case ItemStatus.PURCHASING:
                ...
                if (isSuperAdmin && !request.progressUpdateRequest?.isAcknowledged) {
            ...
            case ItemStatus.IN_DELIVERY:
                 if (isPurchaseAdmin || isLogisticsAdmin) {
...
const NewRequestDetailPage: React.FC<RequestDetailPageProps> = (props) => {
    ...
    const handleAddComment = () => {
        ...
        const updatedRequest = {
            ...request,
            activityLog: [newActivity, ...(request.activityLog || [])]
        };
    ...
    const handleSaveEdit = () => {
        ...
        onUpdateRequest({ ...request, activityLog: updatedLog });
        ...
        >
            <div className="space-y-8">
                <div ref={printRef} className="p-6 bg-white border border-gray-200/80 rounded-xl shadow-sm space-y-8">
                    ...
                    {request.status === ItemStatus.LOGISTIC_APPROVED && currentUser.role === 'Admin Purchase' && (
                    ...
                    {request.purchaseDetails && canViewPrice(currentUser.role) && (
                        <PurchaseDetailsView request={request} details={request.purchaseDetails} />
                    )}
...
const ItemPurchaseDetailsForm: React.FC<ItemPurchaseDetailsFormProps> = ({ item, approvedQuantity, onChange, isDisabled = false }) => {
    ...
    useEffect(() => {
        if (purchaseDate && warrantyPeriod && warrantyPeriod > 0) {
            const d = new Date(purchaseDate);
            // FIX: Explicitly cast getMonth() to number to resolve a TypeScript type inference issue. This fixes an operator error.
