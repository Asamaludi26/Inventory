...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onBack, onSave, prefillData, editingAsset, currentUser, onStartScan, bulkItems, setBulkItems, assetCategories, openModelModal, openTypeModal, setActivePage }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const formData: Omit<RegistrationFormData, 'purchaseDate' | 'registrationDate' | 'warrantyEndDate'> & { purchaseDate: string; registrationDate: string; warrantyEndDate: string | null;} = {
            assetName,
            ...
            purchaseDate: toYYYYMMDD(purchaseDate!),
            registrationDate: toYYYYMMDD(registrationDate!),
            recordedBy: currentUser.name,
            warrantyEndDate: warrantyDate ? toYYYYMMDD(warrantyDate) : null,
            condition,
        ...
        setTimeout(() => { // Simulate API Call
            onSave(formData as any, editingAsset?.id);
            setIsSubmitting(false);
        }, 1000);
    };
                ...
                {canViewPrice(currentUser.role) && (
                    <FormSection title="Informasi Pembelian" icon={<DollarIcon className="w-6 h-6 mr-3 text-tm-primary" />}>
                        ...
                        <div>
                            ...
                            <input type="number" id="warrantyPeriod" value={warrantyPeriod} onChange={e => setWarrantyPeriod(e.target.value === '' ? '' : parseInt(e.target.value, 10))} className="block w-full px-3 py-2 mt-1 text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-300 rounded-md shadow-sm sm:text-sm" placeholder="Contoh: 12" />
                        </div>
                        <div className="md:col-span-2">
                             <label htmlFor="warrantyDate" className="block text-sm font-medium text-gray-700">Tanggal Akhir Garansi (Otomatis)</label>
                            <DatePicker id="warrantyDate" selectedDate={warrantyDate} onDateChange={setWarrantyDate} />
                        </div>
                    </FormSection>
                )}

                <FormSection title="Kondisi & Penempatan Awal" icon={<WrenchIcon className="w-6 h-6 mr-3 text-tm-primary" />}>
                     <div>
                        <label htmlFor="condition" className="block text-sm font-medium text-gray-700">Kondisi Awal</label>
                        <CustomSelect
                    ...
                    <div>
                        <label htmlFor="location" className="block text-sm font-medium text-gray-700">Lokasi Awal</label>
                        <CustomSelect
                    ...
                    <div className="md:col-span-2">
                        <label htmlFor="locationDetail" className="block text-sm font-medium text-gray-700">Detail Lokasi</label>
                        <input type="text" id="locationDetail" value={locationDetail} onChange={e => setLocationDetail(e.target.value)} className="block w-full px-3 py-2 mt-1 text-gray-900 bg-gray-50 border border-gray-300 rounded-md shadow-sm sm:text-sm" placeholder="Contoh: Rak A-01, Meja Kerja Budi"/>
                    </div>
                     <div className="md:col-span-2">
                        <label htmlFor="initialUser" className="block text-sm font-medium text-gray-700">Pengguna Awal (Opsional)</label>
                        <input type="text" id="initialUser" value={initialUser} onChange={e => setInitialUser(e.target.value)} className="block w-full px-3 py-2 mt-1 text-gray-900 bg-gray-50 border border-gray-300 rounded-md shadow-sm sm:text-sm" placeholder="Jika langsung diserahkan ke pengguna" />
                    </div>
                </FormSection>

                <FormSection title="Lampiran & Catatan" icon={<PaperclipIcon className="w-6 h-6 mr-3 text-tm-primary" />}>
                    <div className="md:col-span-2">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <textarea id="notes" value={notes} onChange={e => setNotes(e.target.value)} rows={3} className="block w-full px-3 py-2 mt-1 text-gray-900 bg-gray-50 border border-gray-300 rounded-md shadow-sm sm:text-sm" />
                    </div>
                </FormSection>
...
// Main Component
const ItemRegistration = (props: ItemRegistrationProps) => {
    const { onClearPrefill, onRegistrationComplete, onInitiateHandover, onInitiateDismantle, onInitiateInstallation, assetToViewId, initialFilters, onClearInitialFilters, itemToEdit, onClearItemToEdit, onShowPreview, setActivePage, openModelModal, openTypeModal, setIsGlobalScannerOpen, setScanContext, setFormScanCallback } = props;
    const { currentUser, assets, setAssets, customers, requests, handovers, dismantles, assetCategories } = props;
    ...
    const initialFilterState = { status: '', category: '', condition: '', warranty: '' };
    const [filters, setFilters] = useState(initialFilterState);
    const [tempFilters, setTempFilters] = useState(filters);
    const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
    const filterPanelRef = useRef<HTMLDivElement>(null);
    
    const [bulkAction, setBulkAction] = useState<'delete' | 'handover' | 'install' | null>(null);
    const [bulkHandoverTarget, setBulkHandoverTarget] = useState('');
    const [bulkInstallTarget, setBulkInstallTarget] = useState('');
    
    const [assetToDeleteId, setAssetToDeleteId] = useState<string | null>(null);
    const [assetToDetail, setAssetToDetail] = useState<Asset | null>(null);
    const [qrModalAsset, setQrModalAsset] = useState<Asset | null>(null);
    const [localScannerTarget, setLocalScannerTarget] = useState<{ isOpen: boolean, itemId: number | null }>({ isOpen: false, itemId: null });
    const [bulkItems, setBulkItems] = useState<{ id: number; serialNumber: string; macAddress: string; }[]>([]);

    const [isLoading, setIsLoading] = useState(false);
    const addNotification = useNotification();

    // -- Prefill and Initial State Logic --
    useEffect(() => {
    ...
    useEffect(() => {
        if (assetToViewId) {
            ...
            if (asset) setAssetToDetail(asset);
        }
    }, [assetToViewId, assets]);
    ...
    // -- Event Handlers & Logic for List View --
    ...
    const handleSaveAsset = (formData: RegistrationFormData, assetIdToUpdate?: string) => {
        if (assetIdToUpdate) { // Update logic
            setAssets(prev => prev.map(asset => {
                if (asset.id === assetIdToUpdate) {
                    const newLog: ActivityLogEntry = {
                        ...
                        details: 'Data aset telah diperbarui.'
                    };
                    return {
                        ...asset,
                        ...
                        serialNumber: formData.bulkItems[0]?.serialNumber || undefined,
                        macAddress: formData.bulkItems[0]?.macAddress || undefined,
                        purchasePrice: formData.purchasePrice,
                        vendor: formData.vendor,
                        poNumber: formData.poNumber,
                        invoiceNumber: formData.invoiceNumber,
                        purchaseDate: toYYYYMMDD(formData.purchaseDate),
                        registrationDate: toYYYYMMDD(formData.registrationDate),
                        warrantyEndDate: formData.warrantyEndDate ? toYYYYMMDD(formData.warrantyEndDate) : null,
                        condition: formData.condition,
                        location: formData.location,
                        locationDetail: formData.locationDetail,
                        currentUser: formData.currentUser,
                        notes: formData.notes,
                        attachments: [...asset.attachments, ...formData.attachments],
            ...
            addNotification('Aset berhasil diperbarui!', 'success');
        } else { // Create logic
            const newAssets: Asset[] = formData.bulkItems.map((item, index) => {
                const newAssetId = `AST-${String(assets.length + index + 1).padStart(4, '0')}`;
                const newLog: ActivityLogEntry = {
                    id: `log-${newAssetId}-create`,
                    timestamp: new Date().toISOString(),
                    user: currentUser.name,
                    action: 'Aset Dicatat',
                    details: `Aset baru dicatat dari request ${formData.relatedRequestId || 'manual'}.`
                };
                return {
                    id: newAssetId,
                    name: formData.assetName,
                    ...
                    condition: formData.condition,
                    woRoIntNumber: formData.relatedRequestId || `REG-${newAssetId}`,
                    notes: formData.notes,
            ...
            if (formData.relatedRequestId && props.prefillData?.itemToRegister) {
                onRegistrationComplete(formData.relatedRequestId, { 
                    requestItemId: props.prefillData.itemToRegister.id, 
    ...
    const handleConfirmDelete = () => {
        ...
        setIsLoading(true);
        setTimeout(() => {
            setAssets(prev => prev.filter(a => a.id !== assetToDeleteId));
    ...
    const startLocalScan = (itemId: number) => {
        setLocalScannerTarget({ isOpen: true, itemId });
    };

    const handleLocalScanSuccess = (decodedText: string) => {
        const parsed = parseScanData(decodedText);
        if (localScannerTarget.itemId) {
            setBulkItems(prev => prev.map(item => {
                if (item.id === localScannerTarget.itemId) {
                    return {
                        ...item,
                        serialNumber: parsed.serialNumber || item.serialNumber,
                        macAddress: parsed.macAddress || item.macAddress
                    };
                }
                return item;
            }));
        }
        setLocalScannerTarget({ isOpen: false, itemId: null });
        addNotification('Data berhasil dipindai.', 'success');
    };
    
    const startGlobalScan = (callback: (data: ParsedScanResult) => void) => {
        setScanContext('form');
        setFormScanCallback(() => callback);
        setIsGlobalScannerOpen(true);
    };

    const handleScanForForm = (itemId: number) => {
        const callback = (parsedData: ParsedScanResult) => {
             setBulkItems(prev => prev.map(item => {
                if (item.id === itemId) {
                    return {
                        ...item,
                        serialNumber: parsedData.serialNumber || item.serialNumber,
                        macAddress: parsedData.macAddress || item.macAddress
                    };
                }
                return item;
            }));
             addNotification('Data berhasil dipindai dan dimasukkan ke form.', 'success');
        };
        startGlobalScan(callback);
    };
    ...
    const filteredAssets = useMemo(() => {
        ...
        if (currentUser.role === 'Staff') {
            tempAssets = tempAssets.filter(a => a.currentUser === currentUser.name);
        }
        ...
        .filter(asset => {
            ...
            if (filters.warranty === 'expiring') return warrantyEnd > now && warrantyEnd <= nextMonth;
            if (filters.warranty === 'expired') return warrantyEnd < now;
            return true;
        });
    }, [assets, searchQuery, filters, currentUser]);
    ...
    // ... All other states and functions from the original main component
    ...
    const renderContent = () => {
        if (view === 'form') {
            return <RegistrationForm
                        ...
                        editingAsset={itemToEdit?.data}
                        currentUser={currentUser}
                        onStartScan={handleScanForForm}
                        bulkItems={bulkItems}
                ...
                {/* Header */}
                 <div className="flex flex-col items-start justify-between gap-4 mb-6 md:flex-row md:items-center">
                    ...
                    <div className="flex items-center space-x-2">
                        <button onClick={() => exportToCSV(sortedAssets, 'daftar_aset_triniti.csv')} className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 bg-white border rounded-lg shadow-sm hover:bg-gray-50">
    ...
    return (
        <div className={view === 'form' ? 'pb-24' : ''}>
            ...
            {/* All modals go here */}
            {assetToDetail && (
                <Modal isOpen={!!assetToDetail} onClose={() => setAssetToDetail(null)} title={`Detail Aset: ${assetToDetail.name}`}>
                    <p>Detail Content for {assetToDetail.id}</p>
                </Modal>
            )}

            {assetToDeleteId && (
                <Modal isOpen={!!assetToDeleteId} onClose={() => setAssetToDeleteId(null)} title="Konfirmasi Hapus">
                   <p>Are you sure you want to delete {assetToDeleteId}?</p>
                </Modal>
            )}
        </div>
    );
