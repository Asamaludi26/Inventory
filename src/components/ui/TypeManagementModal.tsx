...
import { AssetCategory, AssetType, Asset, TrackingMethod } from '../../types';
...
export const TypeManagementModal: React.FC<TypeManagementModalProps> = ({
  ...
  const types = parentCategory.types || [];
  ...
  const handleDeleteClick = (type: AssetType) => {
    const assetCount = assets.filter(asset => asset.category === parentCategory.name && asset.type === type.name).length;
    setTypeToDelete({ ...type, assetCount });
  };

  const handleConfirmDelete = () => {
    if (typeToDelete && typeToDelete.assetCount === 0) {
      onDelete(parentCategory, typeToDelete);
      ...
      >
        <div className="p-6 space-y-6">
            <div className="p-3 text-sm text-blue-800 bg-blue-50/70 rounded-lg border border-blue-200/50">
                Mengelola Tipe Aset untuk Kategori: <strong className="font-semibold">{parentCategory.name}</strong>
            </div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar pr-2 -mr-2">
              {types.length > 0 ? (
                types.map(type => {
                    ...
                    const assetCount = assets.filter(a => a.category === parentCategory.name && a.type === type.name).length;
                    return (
